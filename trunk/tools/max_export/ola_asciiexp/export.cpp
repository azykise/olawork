...
void AsciiExp::ExportMesh(INode* node, TimeValue t, int indentLevel)
{
	...
	IGameNode* i_node = mIGame->GetIGameNode(node);
	if( GetIncludeTangents() && i_node)
	{
		IGameObject* i_obj = i_node->GetIGameObject();
		IGameObject::ObjectTypes t = i_obj->GetIGameType();
		IGameMesh* i_mesh = 0;
		if(t == IGameObject::IGAME_MESH)
		{
			i_mesh = static_cast<IGameMesh*>(i_obj);
		}
		else
			goto skip_tange_binormal;

		i_mesh->SetCreateOptimizedNormalList();
		i_mesh->InitializeData();	

		// export mesh (vertex) tangents
		fprintf(pStream, "%s\t%s {\n",indent.data(), ID_MESH_TANGENTS);
		
		Point3 vt;  // vertex tangent
		Point3 vb;  // vertex bnormal
		int  vert;
		Face* f;
		FaceEx* fex;

		// In MAX a vertex can have more than one tangent (but doesn't always have it).
		// This is depending on the face you are accessing the vertex through.
		for (i = 0; i< i_mesh->GetNumberOfFaces(); i++) {

			f = &mesh->faces[i];
			fex = i_mesh->GetFace(i);

			fprintf(pStream,"%s\t\t%s %d\t\n", indent.data(), ID_MESH_FACETANGENT, i);
...
skip_tange_binormal:
	...
	BoneModData* bone_mod = GetBoneModeData(node);
	ISkin* skin = GetSelectedSkinModify(node);

	if ( GetIncludeSkin() && bone_mod && skin)
	{
		int skined_vert_num = bone_mod->GetNumPoints();
		int num_vertex = mesh->getNumVerts();

		if(skined_vert_num != num_vertex)
			goto mesh_end;

		fprintf(pStream, "%s\t%s {\n",indent.data(), ID_MESH_VERTEXBONES);
		fprintf(pStream, "%s\t\t%s %d\n",indent.data(), ID_MESH_VERTEXBONE_NUM,skined_vert_num);
		VertexBoneInfo vbi;
		for (int i = 0 ; i < skined_vert_num ; i++)
		{
			vbi.reset();
			GetVertexBoneInfo(&vbi,i,bone_mod,skin);
			
			fprintf(pStream,"%s\t\t%s %d\t%s\n", indent.data(), ID_MESH_VERTEXBONE, i, Format(vbi));
		}
		
		fprintf(pStream, "%s\t}\n",indent.data());
	}	

	if( GetIncludeSkeleton() && skin)
	{
		if (!skin)
			goto mesh_end;

		int bone_num = skin->GetNumBones();
		if(bone_num == 0)
			goto mesh_end;

		fprintf(pStream, "%s\t%s {\n",indent.data(), ID_MESH_SKELETON);
		fprintf(pStream, "%s\t\t%s %d\n",indent.data(), ID_SKELETON_BONE_NUM,bone_num);
		for (int i = 0 ; i < bone_num ; i++)
		{
			INode* bone = skin->GetBone(i);							
			TCHAR* bone_name = bone->GetName();
			fprintf(pStream, "%s\t\t%s ID:%d NAME:[%s] \n",indent.data(), ID_SKELETON_BONE, i , bone_name );
		}
		fprintf(pStream, "%s\t}\n",indent.data());
	}
...
ISkin* AsciiExp::GetSelectedSkinModify(INode* node)
{
	Object* obj = node->GetObjectRef();		
	if (obj && node->Selected() && obj->SuperClassID()==GEN_DERIVOB_CLASS_ID)
	{
		IDerivedObject* iobj = (IDerivedObject*)obj;
		int nummod = iobj->NumModifiers();
		for (int m=0;m<nummod;++m)
		{
			Modifier *mod = iobj->GetModifier(m);
			if (!strstr(mod->GetName(),"Skin") && !strstr(mod->GetName(),"ÃÉÆ¤") )
				continue;
			ISkin* skin_interface=(ISkin*)mod->GetInterface(I_SKIN);
...
void AsciiExp::DumpMaterial(Mtl* mtl, int mtlID, int subNo, int indentLevel)
{
	...
	if (mtl->ClassID() == Class_ID(DMTL_CLASS_ID, 0)) {
		...
		TSTR fallofout = std->GetFalloffOut()== 1 ? _T("Out") : _T("In");
		fprintf(pStream,"%s\t%s %s\n", indent.data(), _T("*MATERIAL_FALLOFF"), fallofout);
...
TSTR AsciiExp::Format(VertexBoneInfo& value)
{
	TCHAR buf[120];
	
	sprintf(buf, "A:%d %.4f\tB:%d %.4f\tC:%d %.4f\tD:%d %.4f\tO:%d",
		value.boneID[0], value.weights[0],
		value.boneID[1], value.weights[1],
		value.boneID[2], value.weights[2],
		value.boneID[3], value.weights[3],
		value.out4);
...
void AsciiExp::GetVertexBoneInfo(VertexBoneInfo* out,int skined_vid,BoneModData* bone_mod,ISkin* skin)
{	
	int bone_num = bone_mod->GetNumAssignedBones(skined_vid);
	out->out4 = bone_num > 4 ? 1 : 0;
	bone_num = min(bone_num,4);

	for (int bn = 0 ; bn < bone_num ; bn++)
	{
		int bone_id = bone_mod->GetAssignedBone(skined_vid,bn);
		INode* skin_bone = skin->GetBone(bone_id);
		if(!skin_bone)
		{
			continue;;
		}

		out->boneID[bn] = bone_id;		
		out->weights[bn] = bone_mod->GetBoneWeight(skined_vid,bn);
	}
}

BoneModData* AsciiExp::GetBoneModeData(INode* node)
{
	BoneModData* bone_mod = 0;
	Object* obj = node->GetObjectRef();
	SClass_ID s_cid = obj->SuperClassID();
	if (obj && node->Selected() && s_cid == GEN_DERIVOB_CLASS_ID)
	{
		IDerivedObject* iobj = (IDerivedObject*)obj;
		int nummod = iobj->NumModifiers();
		for (int m=0;m<nummod;++m)
		{
			Modifier *mod = iobj->GetModifier(m);
			if (!strstr(mod->GetName(),"Skin"))
				continue;

			ModContext* modcontex = iobj->GetModContext(m);
			LocalModData *localdata = modcontex->localData;
			bone_mod = (BoneModData*)localdata;
		}
	}
	return bone_mod;
}
