...
// you should change!
#define OLA_ASCIIEXP_CLASS_ID	Class_ID(0x3f933a33, 0x3a9c7eab)
...
public:
	...
	Class_ID		ClassID() { return OLA_ASCIIEXP_CLASS_ID; }
...
AsciiExp::~AsciiExp()
{
	SkeletonBoneTable::iterator bone_i = mSkeletonBones.begin();
	while (bone_i != mSkeletonBones.end())
	{
		delete bone_i->second;
		bone_i++;
	}
	...
	WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		CheckDlgButton(hWnd, IDC_SKININFO, exp->GetIncludeSkin());
		CheckDlgButton(hWnd, IDC_SKELETONINFO, exp->GetIncludeSkeleton());
		...
		EnableWindow(GetDlgItem(hWnd, IDC_TANGENTS), exp->GetIncludeMesh());
		EnableWindow(GetDlgItem(hWnd, IDC_SKININFO), exp->GetIncludeMesh());
		break;
	...
	case WM_COMMAND:
		...
		case IDC_MESHDATA:
			...
			EnableWindow(GetDlgItem(hWnd, IDC_VERTEXCOLORS), IsDlgButtonChecked(hWnd,
				IDC_MESHDATA));
			EnableWindow(GetDlgItem(hWnd, IDC_TANGENTS), IsDlgButtonChecked(hWnd,
				IDC_MESHDATA));
			EnableWindow(GetDlgItem(hWnd, IDC_SKININFO), IsDlgButtonChecked(hWnd,
				IDC_MESHDATA));
			break;
		case IDOK:
			...
			exp->SetIncludeSkin(IsDlgButtonChecked(hWnd,IDC_SKININFO));
			exp->SetIncludeSkeleton(IsDlgButtonChecked(hWnd,IDC_SKELETONINFO));
...
int AsciiExp::DoExport(const TCHAR *name,ExpInterface *ei,Interface *i, BOOL suppressPrompts, DWORD options) 
{
	...
	mIGame = GetIGameInterface();

	// Get the options the user selected the last time
	ReadConfig();

	IGameConversionManager* cmng = GetConversionManager();
	cmng->SetCoordSystem(IGameConversionManager::IGAME_MAX);

	mIGame->InitialiseIGame(exportSelected);
